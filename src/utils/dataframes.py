...
def process_data(source_name, dataframe, config, date):
    ...
    sys_logger.info(f"Input dataframe shape: {dataframe.shape}")
    sys_logger.info(f"Input dataframe columns: {dataframe.columns}")
    
    key_cols = config.get("key_columns",[])
    is_full = config.get('data_type') == 'full'

    # Get current table with more logging
    current_table = get_saved_file(source_name)
    sys_logger.info(f"Current table shape: {current_table.shape}")

    # Bypass validate_data for now
    clean_records, error_records = validate_data(source_name, dataframe)

    # More logging in SCD2 application
    ...
    sys_logger.info(f"New table shape before saving: {new_table.shape}")
    
    # Get full save path
    save_path = get_save_filename(source_name)
    sys_logger.info(f"Attempting to save to full path: {save_path}")
